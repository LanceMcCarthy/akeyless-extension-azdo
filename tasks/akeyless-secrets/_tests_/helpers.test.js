...
describe('helpers.js', () => {
  ...
  describe('processStaticSecretResponse', () => {
    ...
    test('should allow multiline static secrets without requiring pipeline env changes', () => {
      ...
      SDK.setVariable = vi.fn((name, value, secret, isOutput) => {
...
-----END RSA PRIVATE KEY-----`;
    ...
    test('should encode multiline static secrets as base64 on non-Windows agents', () => {
      // Arrange
      ...
      const encodedSecret = Buffer.from(multilineSecret, 'utf8').toString('base64');
      const staticSecretsDictionary = {
        '/path/private-key': 'privateKeyOutput'
      };
      const secretResult = {
        '/path/private-key': multilineSecret
      };
      ...
      try {
        ...
        // Assert
        expect(SDK.setVariable).toHaveBeenNthCalledWith(1, 'privateKeyOutput', encodedSecret, true, true);
        expect(SDK.setVariable).toHaveBeenNthCalledWith(2, 'privateKeyOutput_ENCODING', 'base64', false, true);
        expect(process.env.SYSTEM_UNSAFEALLOWMULTILINESECRET).toBeUndefined();
        expect(process.env.SYSTEM_UNSAFE_ALLOW_MULTILINE_SECRET).toBeUndefined();
        expect(console.log).toHaveBeenCalledWith("✅ '/path/private-key' => output: privateKeyOutput (multiline secret stored as base64; see privateKeyOutput_ENCODING)");
      } finally {
        if (originalAllowMultiline !== undefined) {
          process.env.SYSTEM_UNSAFEALLOWMULTILINESECRET = originalAllowMultiline;
        } else {
  ...
  describe('processDynamicSecretResponse', () => {
    ...
    test('should encode multiline autogenerated dynamic values as base64 on non-Windows agents', () => {
      // Arrange
      vi.spyOn(os, 'platform').mockReturnValue('linux');
      const akeylessPath = '/dynamic/secret';
      const outputPrefix = 'myOutput';
      ...
      // Assert
      expect(SDK.setVariable).toHaveBeenNthCalledWith(1, 'myOutput_private_key', Buffer.from('line1\nline2', 'utf8').toString('base64'), true, true);
      expect(SDK.setVariable).toHaveBeenNthCalledWith(2, 'myOutput_private_key_ENCODING', 'base64', false, true);
      expect(console.log).toHaveBeenCalledWith('✅ Output: myOutput_private_key (multiline secret stored as base64; see myOutput_private_key_ENCODING).');
    });
